...
describe('lib/drep/batch', function () {
    describe('execute', function () {
        it('should execute batch request', function (done) {
            ...
            var batch = drep.createBatch(); 
            batch.add(drep.eth.getBalance.request('0x0000000000000000000000000000000000000000', 'latest', callback));
            batch.add(drep.eth.getBalance.request('0x0000000000000000000000000000000000000005', 'latest', callback2));
            batch.execute();
        });
        ...
        it('should execute batch request for async properties', function (done) {
            ...
            var batch = drep.createBatch(); 
            batch.add(drep.eth.getAccounts.request(callback));
            batch.add(drep.net.getPeerCount.request(callback2));
            batch.execute();
        });
        ...
        it('should execute batch request with contract', function (done) {
            ...
            var batch = drep.createBatch(); 
            batch.add(drep.eth.getBalance.request('0x0000000000000000000000000000000000000000', 'latest', callback));
            batch.add(drep.eth.contract(abi).at(address).balance.request(address, callback2));
            provider.injectBatchResults([result, result2]);
            batch.execute();
        });
